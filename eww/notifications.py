        ...
        string = string + f"""
                  (button :class 'notif'
                   (box :orientation 'horizontal' :space-evenly false
                      (image :image-width 60 :image-height 60 :path '{item.icon or './notification_icon.svg'}')
                      (box :orientation 'vertical' :space-evenly false :spacing 5
                        (label :class 'heading' :width 125 :justify 'left' :wrap true :xalign 0 :text '{item.summary or ''}')
                        (label :class 'body' :width 125 :justify 'left' :wrap true :xalign 0 :text '{item.body or ''}')
                  )))
                  """
