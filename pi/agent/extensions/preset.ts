...
interface Preset {
	...
	tools?: string[];
	/** Markdown filename containing instructions. Defaults to <preset-name>.md. */
...
function getInstructionsFile(name: string, preset: Preset): string | undefined {
	const fileName = preset.instructionsFile ?? `${name}.md`;

	// Instruction files live in the presets/ directory; do not allow a preset to escape it.
	if (basename(fileName) !== fileName || !fileName.endsWith(".md")) {
		console.error(`Preset "${name}": instructionsFile must be a Markdown filename, got "${fileName}"`);
		return undefined;
	}
...
/** Load the project-local file first, then fall back to the global one. */
function loadInstructions(name: string, preset: Preset, cwd: string): string | undefined {
	const fileName = getInstructionsFile(name, preset);
	if (!fileName) return undefined;

	const paths = [join(cwd, CONFIG_DIR_NAME, "presets", fileName), join(getAgentDir(), "presets", fileName)];
	for (const path of paths) {
		if (!existsSync(path)) continue;
		try {
			return readFileSync(path, "utf-8");
		} catch (err) {
			console.error(`Failed to load instructions for preset "${name}" from ${path}: ${err}`);
		}
	}
...
export default function presetExtension(pi: ExtensionAPI) {
	 ...
	 */
	function buildPresetDescription(name: string, preset: Preset): string {
		const parts: string[] = [];
		...
		const instructionsFile = getInstructionsFile(name, preset);
		if (instructionsFile) {
			parts.push(`instructions:${instructionsFile}`);
		}
	...
	async function showPresetSelector(ctx: ExtensionContext): Promise<void> {
		...
		const items: SelectItem[] = presetNames.map((name) => {
			...
			return {
				...
				description: buildPresetDescription(name, preset),
			};
		});
	...
	pi.on("before_agent_start", async (event, ctx) => {
		if (activePresetName && activePreset) {
			const instructions = loadInstructions(activePresetName, activePreset, ctx.cwd);
			if (instructions) {
				return {
